...
class VNModel():
    def __init__(self):
        ...
        self.ccmodel = surprise.prediction_algorithms.co_clustering.CoClustering(n_cltr_u=3,n_cltr_i=5,n_epochs=5,verbose=True)
    ...
    def top_predictions(self,ruid): #raw user id
        ...
        if ruid in self.user_ser[self.user_ser]:
            ...
            if self.vnc.is_valid():
               votes = pd.DataFrame(self.vnc.get_user_votes(ruid),columns=['user_id', 'VN_id', 'vote'])
               votes = votes[votes['VN_id'].isin(self.ser.keys()[self.ser])]#[votes[votes['VN_id'].isin(self.ser.keys()[self.ser])]['user_id'].isin(self.user_ser.keys()[self.user_ser])] oh i don't need to filter the userids
